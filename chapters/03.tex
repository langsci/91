...
\z
% ACQUISITION 151
In fact, the only reliable indicator of the SNSD is not a single article use, but a series of articles uses; an \textit{a}-\textit{a} sequence, as in \REF{ex:3:10} above, as opposed to an \textit{a}-\textit{the} sequence, as in \REF{ex:3:9} above.
...
\todo{I have introduced langinfo in the next 2 ex. However, it produces weird semicola}
\ea\label{ex:3:22}
...
\ea\label{ex:3:23}
\langinfo{Italian}{Antinucci and Miller's 90}{}\\
...
First, however, we should ask how the cases of Turkish and Kaluli relate to the creole case. We saw in \chapref{ch:2} that out of the six potential strategies for expressing the CNCD described above (case-marking, verbal affixation, causal-verb periphrasis, passivization, lexical
%\originalpage{199}
...
This process does not limit itself to intransitives. Transitive verbs like \textit{eat} which are restricted to noncausative meanings (see \REF{ex:3:70}, \REF{ex:3:71} above) and hence, except where cannibalism is practised\todo{Isn't it ``practiced''?}, to nonhuman objects are also treated as if they were potential causatives:
...
\end{enumerate}
\todo{need to work on formatting here}
