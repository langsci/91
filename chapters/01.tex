...
The first condition rules out Tok Pisin and perhaps other (e.g., Austra\-lian Aboriginal) creoles; the second rules out \ili{Réunion Creole} and perhaps other creoles also (the varieties of \ili{Portuguese creoles} that evolved in Asian trading enclaves such as \isi{Goa} or \isi{Macao} are possible candidates for exclusion under this condition). Given the above, I shall continue to refer to certain languages or groups of languages as ``English creoles,{\textquotedbl} ``French creoles,'' etc.; this usage implies no conclusions as to the affiliations of these languages and is merely for convenience.\is{creole!definition of}
...
Finally, for the fifth area, involving \isi{relativization}, subject-copying, and their interaction, we might need a rule for Filipino speakers\is{Filipino pidgin speakers} which would modify \REF{ex:97} to something like \REF{ex:109}:
